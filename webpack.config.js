...
// removed firebase-tools; using Supabase config file instead
...
function extensionConfig (params = {}) {
  const plugins = [
    ...
    new webpack.DefinePlugin({
      ...
      SUPABASE_CONFIG: JSON.stringify(params.supabaseConfig),
...
export default async function (env) {
  ...
  let supabaseConfig = defaultSupabaseConfig
  if (env.mode !== 'development') {
    const supabaseConfigFile = `./.supabase-config-${env.mode}.json`
    try {
      supabaseConfig = JSON.parse(fs.readFileSync(supabaseConfigFile, 'utf8'))
  } catch {
      // eslint-disable-next-line
      console.warn('Missing', supabaseConfigFile, '- using empty Supabase config')
    }
  }
